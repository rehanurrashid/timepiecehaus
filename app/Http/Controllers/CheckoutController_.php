...
class CheckoutController extends Controller
{
    ...
    public function submitOrder(Request $request, Product $product)
    {
        try {
            ...
            $description = "Congratulations! ".auth()->user()->getFullName()." Your have one request against Order No. TP#$order->id!";
            $action = url('/my-account#order-requests');
            ...
            DB::rollBack();
            Log::error('Error: ' . $ex->getMessage() . ', data: ' . $request->all());
    ...
    {
        $order = $order->with(['product', 'vendor', 'user', 'status'])->first();
        if ($order) {
